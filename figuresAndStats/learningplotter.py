...
from backend.Segmenter import Detector
...
def ComputeLoss(modelPath):
    ...
    for (image, segmentation) in zip(images, segmentations):
        predicted = segmenter.Detect(image.frames[0])
...
def ComputeLossInvert(modelPaths):
    segmenters = [Detector(modelPath) for modelPath in modelPaths]
...
def ComputeLossBatch(modelPath):
    ...
    predicted = segmenter.DetectMultiple(images)
